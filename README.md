...
* Initial program will simply randomly distribute the guests amongst the hosts, giving each the same amount.
  * Add functionality: Each host gets a certain number of guests
    * Add functionality: Guests should not be with the same guest or host twice, if possible
      * Add functionality: Implement different numbers of guests kept each night by different hosts
        * Add functionality????
          * Change input…?? Store guests/hosts in a text file, database, etc
          * Create GUI… web?
